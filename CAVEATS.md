...
>
> **🔴 2026-07-14 Phase 3 resolution (Task T-Interp-2): provenance question CLOSED — confirmed reverse-engineered.**
> Direct git archaeology (`git log -S "33.6255"`, full diff inspection) traces both $\tau$ values to commit `c98b7ec` (2026-06-25), which added `scripts/mass_from_first_principles.py`. That commit's own code fixes `target_mass_S12=3.18e-21`, `target_mass_S21=1.83e-21` and `V=1e4` as literals, then runs `scipy.optimize.minimize` to **solve for the $\tau$ that reproduces the pre-chosen target mass** — the exact $\tau=33.6255/33.8014$ output is what later got hardcoded into `k3_sieve_analysis.py`. A same-day follow-up commit (`400ecdd`) rewrote `mass_from_first_principles.py` to explicitly disavow this procedure as "logically circular" — but that fix never propagated to `k3_sieve_analysis.py`, which still uses the tainted values as of this writing. **The provenance question is no longer open: these $\tau$ values are confirmed products of curve-fitting to a pre-chosen target, not a geometric derivation.** Full trail: `OPEN_PROBLEMS.md` "GAP-2 τ-PROVENANCE RESOLUTION (2026-07-14)".

---

## 2. K3 Surface Identification Is Conjectural (GAP-1)

> **🔴 2026-07-14 Phase 8 rectification — S₁,₂ and S₂,₁ are both elliptic, not K3 surfaces.** Full write-up: `docs/autoresearch_v2/PHASE_8_AUTOEVOLVE_RECTIFICATION.md`. Summary: the Phase 8 exact-rational autonomous sieve re-derived the minimal **generating-function** Picard–Fuchs operators and found that both sequences satisfy an **order-2 ODE** — the signature of an **elliptic curve period**, not a K3 surface. In addition, the $S_{1,2}$ mirror map on the minimal operator has a non-integral second coefficient **$q_2 = 81/8$**, which independently violates the K3-period integrality condition. The earlier `scripts/mirror_map_integrality.py` pass for $S_{1,2}$ was an artifact of using the non-minimal order-3 shift operator. The v1 classifier's bug was using **shift-recurrence order** as the geometry proxy; the correct discriminator is **generating-function ODE order**.
>
> The K3 candidate pool is now the **GATE-C finalists**: $S(2,2)$=A005259 (Apéry $\zeta(3)$), Cooper $s_7$=A183204, Cooper $s_{10}$=A005260, t103=A276536, Domb=A002895, and Almkvist–Zagier second=A125143. These all pass the three algebraic K3 gates: order-3 ODE, integral mirror map, and Fuchsian monodromy.

Neither $S_{1,2}$ nor $S_{2,1}$ should be cited as a K3 surface. Both are kept as **elliptic recurrence-invariants** for the stiffness ratio $\sqrt{1014/336}$.

- The mapping **"Order-3 Picard-Fuchs ↔ K3 Surface"** relies on the Stienstra–Beukers correspondence, which is **not formally verified in Lean 4** and requires additional checks (monodromy computation, modularity of Fourier coefficients). The formalization in this repository proves only concrete values and recurrences, not K3 status.
- The monodromy matrix computation in `scripts/k3_monodromy_verification.py` extracts the PF ODE singular points; the Fuchs-criterion regularity classifier was fixed in 2026-07-11. For the GATE-C K3-type candidates the singular points are regular and MUM at $z=0$; for $S_{1,2}$/$S_{2,1}$ the non-$z=0$ singular points are irregular (consistent with the minimal operator being an elliptic-type ODE with apparent-singularity artifacts rather than a true K3 operator).
- The Weil bound check ($|a_p| \le 2p$, weight-3) passes for the promoted K3 candidates (`scripts/modularity_screen.py`). The earlier Weil-bound comparison between $S_{1,2}$ and $S_{2,1}$ does not discriminate; the decisive evidence is the ODE order and the mirror-map integrality.
- The v1 `scripts/mirror_map_integrality.py` result was **non-minimal-operator**; the Phase 8.B G1-3 gate recomputes on the minimal operator and gives the correct integrality verdict.

**2026-07-14 Phase 8 resolution:** The K3 identification is **re-anchored to the GATE-C pool**. The mass-ratio prediction $\sqrt{1014/336}$ remains arithmetically valid as a statement about the two elliptic sequences' stiffness integers; the geometric interpretation is no longer a K3 mass ratio. See `docs/autoresearch_v2/PHASE_8_AUTOEVOLVE_RECTIFICATION.md` for the full rectification and `data/autoresearch_v2/GATE_C_DECISION.md` for the Phase 8.D deliverables.

**Status**: $S_{1,2}$ and $S_{2,1}$ are **confirmed elliptic-type** (order-2 ODE, $S_{1,2}$ non-integral $q_2$). The K3 candidates are the **GATE-C finalists** listed above. No top-down string compactification or moduli stabilization is established for any candidate.
