...
def build_c3b_recurrence(shift_coeffs, order, degree):
    """
    Convert the exact shift recurrence coeffs from find_shift_recurrence into the
    Python recurrence string expected by check_C3b_symsqrt. The shift recurrence is
        sum_{i=0}^{order} P_i(n) u[n+i] = 0,   P_i(n) = sum_{j=0}^{degree} c_{i,j} n^j
    with coeffs laid out as coeff[i*(degree+1)+j]. The C3b convention is
        C(k) a_{k+1} = A(k) a_k + B(k) a_{k-1}
    where, setting n = k-1:
        C(k) =  P_2(k-1)
        A(k)  = -P_1(k-1)
        B(k)  = -P_0(k-1)
    We flip the global sign if needed so that the leading coefficient of C is positive
    and MUM-normalised to (k+1)^2 for order-2 and (k+1)^3 for order-3.
    """
    k = sp.Symbol("k")
    n = k - 1
    ...
    for i in range(order + 1):
        poly = sum(int(shift_coeffs[i * (degree + 1) + j]) * n ** j for j in range(degree + 1))
        P.append(sp.expand(poly))

    C = sp.expand(P[-1])
    A = sp.expand(-P[-2])
    B = sp.expand(-P[-3]) if order == 2 else sp.Integer(0)

    # Normalise sign: C must have positive leading coefficient.
    C_poly = sp.Poly(C, k)
    if C_poly.LC() < 0:
        C, A, B = -C, -A, -B

    def expr_str(e):
        return str(sp.expand(e)).replace("k", "k")

    if order == 2:
        rec_str = f"(({expr_str(A)})*s[-1] + ({expr_str(B)})*s[-2]) / ({expr_str(C)})"
    else:
        # Not expected for K3-type; fall back to a generic string.
        rec_str = f"(({expr_str(A)})*s[-1] + ({expr_str(B)})*s[-2] + (0)*s[-3]) / ({expr_str(C)})"
    return rec_str


def run_c3b_generated(cid, terms, shift_rec, initial_terms=None):
    """
    Run C3b on a generated (non-pool) candidate by building a temporary refs JSON
    from its exact shift recurrence.
    """
    if not shift_rec or shift_rec.get("order") != 2:
        return {"verdict": "ERROR_SHIFT_RECURRENCE_NOT_ORDER2"}
    degree = shift_rec["degree"]
    coeffs = shift_rec["coeffs"]
    rec_str = build_c3b_recurrence(coeffs, 2, degree)

    init = initial_terms if initial_terms is not None else [str(t) for t in terms[:10]]
    refs = {
        "metadata": {
            "description": "temporary refs for generated candidate",
            "version": "temp",
            "date_created": "",
            "authority": "generated",
            "references": {cid: "generated by AutoEvolve"},
        },
        "sequences": {
            cid: {
                "type": "order-3",
    ...
    try:
        result, code = check_C3b_symsqrt.run_check(temp_path, cid, n_fit=26, n_val=60, deg=2)
    finally:
        try:
            temp_path.unlink()
        except OSError:
    ...
    cert_path = CERT_DIR / f"C3b_symsqrt_{cid}.json"
    result["provenance"] = (
        "Generated-by: scripts/auto_evolve_k3_selection.py (temp refs) | "
        "Verified-by: checkers/check_C3b_symsqrt.py | Reviewed-by: pending T0"
    )
    with open(cert_path, "w") as f:
        json.dump(result, f, indent=2, sort_keys=True, default=str)
