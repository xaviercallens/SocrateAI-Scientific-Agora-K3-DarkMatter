...
**Scope:** settle the S₁₂/S₂₁ definition/geometry conflict by collecting every authoritative definition in this repository, re-running the exact-rational ODE classifier independently, and stating the rigorous alignment.
...
- `scripts/mirror_map_integrality.py:134`
...
## 3. Independent re-run (this session)
...
## 4. What actually exists in Lean 4
...
## 6. Real alignment statement

| object | definition | ODE order | geometry | status |
|---|---|---|---|---|
| S₁₂ | `Σ C(n,k)^1 · C(n+k,k)^2` | 2 | elliptic (weight 2) | **not** a K3 surface under this repo's exact-rational classifier |
| S₂₁ | `Σ C(n,k)^2 · C(n+k,k)^1` | 2 | elliptic (weight 2) | **not** a K3 surface; designated negative control |
| S₂₂ / A005259 | `Σ C(n,k)^2 · C(n+k,k)^2` | 3 | K3-type | literature K3 anchor of the programme |
| Cooper s₇ / A183204 | `Σ C(n,j)^2 · C(2j,n) · C(j+n,j)` | **ODE** 3 | K3-type | GATE-C finalist, Lean kernel-verified |
| Cooper s₁₀ / A005260 | Cooper 10th sporadic | **ODE** 3 | K3-type | GATE-C finalist, Lean kernel-verified |
| T103 / A276536 | `Σ C(n,k) · C(2k,k)^3` | **ODE** 3 | K3-type | GATE-C finalist, Lean kernel-verified |
...
3. Optionally update `PARAMETER_LEDGER.yaml` caveats to remove the stale "S₁₂ remains sole surviving K3" language and replace it with the exact-rational ODE classification.
